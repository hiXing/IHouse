package com.sky.widget.material.switchbutton;
...
import com.sky.widget.material.switchbutton.AnimationController.OnAnimateListener;
...
public class MaterialSwitchButton extends CompoundButton {
	...
	@SuppressLint("NewApi")
	public MaterialSwitchButton(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initView();

		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MaterialSwitchButton);

		mConf.setThumbMarginInPixel(ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_thumb_margin, mConf.getDefaultThumbMarginInPixel()));
		mConf.setThumbMarginInPixel(ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_thumb_marginTop, mConf.getThumbMarginTop()),
				ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_thumb_marginBottom, mConf.getThumbMarginBottom()),
				ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_thumb_marginLeft, mConf.getThumbMarginLeft()),
				ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_thumb_marginRight, mConf.getThumbMarginRight()));
		mConf.setRadius(ta.getInt(R.styleable.MaterialSwitchButton_radius, Configuration.Default.DEFAULT_RADIUS));

		mConf.setThumbWidthAndHeightInPixel(ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_thumb_width, -1), ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_thumb_height, -1));

		mConf.setMeasureFactor(ta.getFloat(R.styleable.MaterialSwitchButton_measureFactor, -1));

		mConf.setInsetBounds(ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_insetLeft, 0), ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_insetTop, 0),
				ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_insetRight, 0), ta.getDimensionPixelSize(R.styleable.MaterialSwitchButton_insetBottom, 0));

		int velocity = ta.getInteger(R.styleable.MaterialSwitchButton_animationVelocity, -1);
	...
	public MaterialSwitchButton(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public MaterialSwitchButton(Context context) {
		this(context, null);
	}
	...
	private void fetchDrawableFromAttr(TypedArray ta) {
		...
		mConf.setOffDrawable(fetchDrawable(ta, R.styleable.MaterialSwitchButton_offDrawable, R.styleable.MaterialSwitchButton_offColor, Configuration.Default.DEFAULT_OFF_COLOR));
		mConf.setOnDrawable(fetchDrawable(ta, R.styleable.MaterialSwitchButton_onDrawable, R.styleable.MaterialSwitchButton_onColor, Configuration.Default.DEFAULT_ON_COLOR));
	...
	private Drawable fetchThumbDrawable(TypedArray ta) {
		...
		Drawable tempDrawable = ta.getDrawable(R.styleable.MaterialSwitchButton_thumbDrawable);
		if (tempDrawable != null) {
			return tempDrawable;
		}

		int normalColor = ta.getColor(R.styleable.MaterialSwitchButton_thumbColor, Configuration.Default.DEFAULT_THUMB_COLOR);
		int pressedColor = ta.getColor(R.styleable.MaterialSwitchButton_thumbPressedColor, Configuration.Default.DEFAULT_THUMB_PRESSED_COLOR);
